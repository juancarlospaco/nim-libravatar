...
- Uses `func` so is Side Effects Free, works with Async & MultiSync, `runnableExamples` included, Self-Documented, NEP1 Ok.
- Best quality avatar sizes are between `80` & `128`, `512` may or may not look a little stretched, also depends on original quality.
